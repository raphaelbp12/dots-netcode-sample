...
{
    public uint tick;
    private int LifeTimeComponentValue;
    private int SpeedComponentValue;
    ...
    public float GetLifeTimeComponentValue(GhostDeserializerState deserializerState)
    {
        return LifeTimeComponentValue * 0.001f;
    }
    public float GetLifeTimeComponentValue()
    {
        return LifeTimeComponentValue * 0.001f;
    }
    public void SetLifeTimeComponentValue(float val, GhostSerializerState serializerState)
    {
        LifeTimeComponentValue = (int)(val * 1000);
    }
    public void SetLifeTimeComponentValue(float val)
    {
        LifeTimeComponentValue = (int)(val * 1000);
    }
    public float GetSpeedComponentValue(GhostDeserializerState deserializerState)
    {
        return SpeedComponentValue * 0.001f;
    }
    public float GetSpeedComponentValue()
    {
        return SpeedComponentValue * 0.001f;
    }
    public void SetSpeedComponentValue(float val, GhostSerializerState serializerState)
    {
        SpeedComponentValue = (int)(val * 1000);
    }
    public void SetSpeedComponentValue(float val)
    {
        SpeedComponentValue = (int)(val * 1000);
    }
    ...
    {
        changeMask0 = (LifeTimeComponentValue != baseline.LifeTimeComponentValue) ? 1u : 0;
        changeMask0 |= (SpeedComponentValue != baseline.SpeedComponentValue) ? (1u<<1) : 0;
        ...
        if ((changeMask0 & (1 << 0)) != 0)
            writer.WritePackedIntDelta(LifeTimeComponentValue, baseline.LifeTimeComponentValue, compressionModel);
        if ((changeMask0 & (1 << 1)) != 0)
            writer.WritePackedIntDelta(SpeedComponentValue, baseline.SpeedComponentValue, compressionModel);
        ...
        NetworkCompressionModel compressionModel)
    {
        ...
        if ((changeMask0 & (1 << 0)) != 0)
            LifeTimeComponentValue = reader.ReadPackedIntDelta(baseline.LifeTimeComponentValue, compressionModel);
        else
            LifeTimeComponentValue = baseline.LifeTimeComponentValue;
        if ((changeMask0 & (1 << 1)) != 0)
            SpeedComponentValue = reader.ReadPackedIntDelta(baseline.SpeedComponentValue, compressionModel);
        else
            SpeedComponentValue = baseline.SpeedComponentValue;
        if ((changeMask0 & (1 << 2)) != 0)
        {
    ...
    {
        SetLifeTimeComponentValue(math.lerp(GetLifeTimeComponentValue(), target.GetLifeTimeComponentValue(), factor));
        SetSpeedComponentValue(math.lerp(GetSpeedComponentValue(), target.GetSpeedComponentValue(), factor));
